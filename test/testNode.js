...
describe('@luxedo/heatmap', () => {
  describe('drawGeoHeatmap', () => {
    ...
    it('should return an image buffer and two coordinates', () => {
      ...
      } = heatmap.drawGeoHeatmap(coords, points1, color1, color2, true, scale)
    ...
    it('should return an image with fixed width when passing widht as argument', () => {
      ...
      const {
        buf
      } = heatmap.drawGeoHeatmap(coords, points1, color1, color2, true, null, width)
      const img = new Image()
      img.src = buf
    ...
    it('should return an image with fixed height when passing height as argument', () => {
      ...
      const {
        buf
      } = heatmap.drawGeoHeatmap(coords, points1, color1, color2, true, null, null, height)
      const img = new Image()
      img.src = buf
    ...
    it('should return an all green image when not passing points', () => {
      const {
        buf
      } = heatmap.drawGeoHeatmap(coords, [], color1, color2, false, scale)
      const imgData = bufToImgData(buf)
      for (let i = 0; i < imgData.data.length; i += 4) {
        const px = [imgData.data[i + 0], imgData.data[i + 1], imgData.data[i + 2], imgData.data[i + 3]]
    ...
    it('should return an all red image when passing a huge point', () => {
      ...
      const {
        buf
      } = heatmap.drawGeoHeatmap(coords, points2, color1, color2, false, scale)
      const imgData = bufToImgData(buf)
      for (let i = 0; i < imgData.data.length; i += 4) {
        const px = [imgData.data[i + 0], imgData.data[i + 1], imgData.data[i + 2], imgData.data[i + 3]]
